...
def main():
    ...
    # Name of this script
    name_of_script = 'Backup_DA_Fire_Folder.py'
    ...
    #  any mapped drives.
    # i.e. D:\users\mgrue\...\...\
    folder_to_backup          = r'{}\Damage_Assessment_GIS\Fire_Damage_Assessment\DEV'.format(path_prefix)
    folder_to_contain_backups = r'{}\Damage_Assessment_GIS\Fire_Damage_Assessment\DEV_backups'.format(path_prefix)
    log_file                  = r'{}\Damage_Assessment_GIS\Fire_Damage_Assessment\DEV\Scripts\Logs\{}'.format(path_prefix, name_of_script.split('.')[0])
    cfgFile                   = r"{}\Damage_Assessment_GIS\Fire_Damage_Assessment\DEV\Scripts\Config_Files\config_file.ini".format(path_prefix)  # With email account info
    ...
    if len(existing_backups) > 0:
        print '  There are "{}" existing backups.  "{}" are allowed:'.format(len(existing_backups), number_of_backups_allowed)
...
def Email_W_Body(subj, body, email_list, cfgFile=
    ...
    config.read(cfgFile)
    print 'Using Config File at: {}'.format(cfgFile)
